...
int main() {
    ...
    // Print the loaded data for verification (you can remove this in the final code)
    for (int i = 0; i < numLines; i++) {
        printf("%d %s \"%s\"\n", ioRequests[i].clk, ioRequests[i].operation, ioRequests[i].message);
    }

    free(ioRequests); // Don't forget to free the allocated memory
